...
{
    Next(Vec<(Arc<FnNext<SS, Ref<V>>+'o>, UnsafeCell<Option<Box<FnErrCompBox<SS, Ref<E>> +'o>>>, Unsub<'o, SS>)>),
    Error(E),
    Complete,
    Drop
}
...
impl<'o, V, E:Clone, SS:YesNo> Subject<'o, SS, V, E>
{
    ...
    #[inline(never)]
    fn sub_internal(&self, next: Arc<FnNext<SS, Ref<V>>+'o>, ec: Box<FnErrCompBox<SS, Ref<E>> +'o>, make_sub: impl FnOnce()->Unsub<'o, SS>) -> Unsub<'o, SS>
    {
    ...
    #[inline(never)]
    fn unsub(state: Weak<Wrap<'o,SS,V,E>>, observer: Weak<FnNext<SS, Ref<V>>+'o>)
    {
...
{
    fn sub_dyn(&self, next: Box<FnNext<NO, Ref<V>>+'o>, ec: Box<FnErrCompBox<NO, Ref<E>> +'o>) -> Unsub<'o, NO>
    {
        let next: Arc<FnNext<NO, Ref<V>>+'o> = next.into();
